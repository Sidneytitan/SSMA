...
def save_data_to_excel(driver_list, filial, operacao, periodo_infra, monitor_name):
    # Cria um DataFrame com os dados dos motoristas
    df = pd.DataFrame(driver_list)

    # Adiciona informações de filial, operação e motorista monitor como colunas extras
    df['Filial'] = filial
    df['Operação'] = operacao
    df['Período de Infrações'] = f"{periodo_infra[0].strftime('%d/%m/%Y')} - {periodo_infra[1].strftime('%d/%m/%Y')}"
    df['Motorista Monitor'] = monitor_name
...
def app():
    ...
    if st.session_state['logged_in']:
        ...
        option = st.sidebar.radio("", ["Cadastro de Infrações", "Visualizar Tabela"])
        ...
        if option == 'Cadastro de Infrações':
            ...
            with col2:
                infracao_options = ['Sem EPI', 'Excesso veloc. PM','Excesso veloc. PS', 'Celular','Usar Celular ao Volante','CT-Entre 94% e 80%','Sem Cinto de Seguranca']
            ...
            monitor_name_options = ['ALEX LUIZ DE SOUZA NUNES', 'WANDERLEI CARLOS BOGAS SARAIVA', 'JOVAIR ROSA RODRIGUES']
            ...
            if st.button('Gerar PDF e Salvar Excel'):
                if st.session_state.driver_list:
                    ...
                    excel_file_name = save_data_to_excel(st.session_state.driver_list, filial, operacao, periodo_infra,
                                                         monitor_name)

                    st.download_button(label="Baixar PDF", data=pdf, file_name="SSMA_pdf.pdf", mime="application/pdf")
                    st.success("Dados salvos com sucesso!")

        elif option == 'Visualizar Tabela':
            # Tabela de motoristas
            st.title('Tabela de Motoristas')
            if 'logged_in' in st.session_state and st.session_state['logged_in']:
                df = pd.read_excel('motoristas.xlsx')
                st.write(df)
            else:
                st.error("Faça login para visualizar a tabela de motoristas.")
